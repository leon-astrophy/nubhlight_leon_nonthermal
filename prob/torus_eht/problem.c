...
void init_prob()
{
  ...
  //////////////////////////////////////////////////
  // Inner edge and radius of pressure maximum //
  // Plasma beta for initial magnetic field //
  // Leon: make it changable parameter // 
  // Leon: not sure why rin is not read properly //
  rin = 21.;
  rmax = 40.;
  beta = 1.e2;
  //////////////////////////////////////////////////
  ...
  ZSLOOP(-1, N1, -1, N2, -1, N3) {
    ...
    // Sine and cosine 90 deg? //
    sthin = sin(thin);
    cthin = cos(thin);

    // may need to read the initial FM paper //
     ...
     * so it needs to be transformed at the end */
      ...
      // zero radial and theta velocity //
      ur = 0.;
      uh = 0.;
  ...
  ZSLOOP(0, N1, 0, N2, 0, 0) {
    ...
    // why find coordinate here? //
    coord(i, j, k, CORN, X);
    bl_coord(X, &r, &th);

    // Choose between MAD and SANE //
    if (MAD == 0) {
      q = rho_av/rhomax - 0.2;
    } else if (MAD == 1) {
  ...
  ZLOOP {
    ...
    // local bsquare, and find maximum across domain //
...
/******************************************************************************/
/* angular momentum of a FM torus */
