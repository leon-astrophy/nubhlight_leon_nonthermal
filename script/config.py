...
def print_config(key, var):
  print("    " + util.color.BOLD + "{:<15}".format(key) + util.color.NORMAL + str(var))
...
#-----------------------------------------------------------------------------#
# check if compile-time paramter is active #
...
#-----------------------------------------------------------------------------#
# check if build using debug #
def is_user_debug():
  return '-debug' in sys.argv

#-----------------------------------------------------------------------------#
# check if build using force #
def is_user_force():
  return '-force' in sys.argv

#-----------------------------------------------------------------------------#
# check if build without parameters #
def is_user_noparam():
  return '-noparam' in sys.argv

#-----------------------------------------------------------------------------#
# check if build with no clean #
def is_user_noclean():
  return '-noclean' in sys.argv

#-----------------------------------------------------------------------------#
# check if build with help #
def is_user_help():
...
def build(PROBLEM, PATHS):
  ...
  # check if debug compiler option is set #
  ...
  # set version of the code to compile time parameter #
  ...
  # print to terminal, also set compile time parameters #
  ...
  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
  # Leon: need to set unit for these four conditions #
  NEED_UNITS = (util.parm_is_active(CPARMS, 'NONTHERMAL') or
                util.parm_is_active(CPARMS, 'RADIATION') or
  ...
  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
  #-------------------------------------------------------#
  # section for nonthermal electrons #
  # range of nonthermal energy injection #
  if util.parm_is_active(CPARMS, 'NONTHERMAL'):
    set_rparm('gammainjmin', 'double', default = 500)
    set_rparm('gammainjmax', 'double', default = 5e5)
  #-------------------------------------------------------#  
  ...
  set_rparm('cour', 'double', default = 0.7)
  if util.parm_is_active(CPARMS, 'RADIATION'):
  ...
  #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$#
  # WRITE Compile time PARAMETERS FILE
