...
double conservative_dtau_est(double nu, int type, int interaction, const struct of_microphysics *m) {
...
/***************************************************************************/
/* bias estimate */
double bound_bias(double bias, double nu, int type, int interaction, const struct of_microphysics *m, double uph) {
...
/***************************************************************************/
/* scattering bias */
double get_scatt_bias(double nu, int type, int interaction, const struct of_microphysics *m, double uph) {
...
/***************************************************************************/
/* not sure what it is */
void rebalance_biases(double dtau_scatt[RAD_SCATT_TYPES], double bias_scatt[RAD_SCATT_TYPES]) {

  /* return if only one type of scattering */
  ...
  // loop over all type of scattering particles //
  SCATTLOOP {
 ...
 * 
 ***************************************************************************/
void bound_all_biases(double dtau_scatt[RAD_SCATT_TYPES], double bias_scatt[RAD_SCATT_TYPES], 
                      double nu, double type, const struct of_microphysics *m) {
  ...
  SCATTLOOP {
    ...
    if (bias_scatt[iscatt] < 1.)
      bias_scatt[iscatt] = 1.;
    double dtau = dtau_scatt[iscatt]; 
    ///////////////////////////////////////////////
    // conservative_dtau_est(nu,type,iscatt,m);
    ///////////////////////////////////////////////
  ...
  // Shrink all biases so sum is safe //
  if (ratio < 1) { 
  ...
  SCATTLOOP {
    if (isnan(bias_scatt[iscatt]) || bias_scatt[iscatt] < 1 ||isinf(bias_scatt[iscatt])) {
...
/***************************************************************************/
/* scattering between superphoton and particles */
...
#pragma omp parallel
  {
    ...
    // initialize optical depth and bias //
    SCATTLOOP {
      dtau_scatt[iscatt] = 0.;
      bias_scatt[iscatt] = 0.;
    }

    // loop over non-null photton //
    while (ph != NULL) {

      // neglect tracers or small weight photon //
      ...
      while (nint < MAX_INTERACTIONS) {
        ...
        // increase number of interaction //
        nint++;

        // get time coordinate and interpolate //
        double dtint  = t + dt - tmin;
        int    status = get_X_K_interp(ph, t + dt / 2., P, X, Kcov, Kcon);

        // skip failed interpolation //
        ...
        // skip out-of-domain photon //
        if (i < 0 || i > N1 + 2 * NG - 1 || j < 0 || j > N2 + 2 * NG - 1 || k < 0 || k > N3 + 2 * NG - 1) {
...
#pragma omp atomic
        ...
        */
        ...
        // skip if the scattering particle has too less temperature //
...
#pragma omp atomic
        ...
        */
        ...
        double theta = get_bk_angle( X, Kcon, Ucov_grd[i][j][k], Bcov_grd[i][j][k], m_grd[i][j][k].B);
        dtau_abs = ((HPL * L_unit / (ME * CL * CL)) * dlam * alpha_inv_abs(nu, ph->type, &(m_grd[i][j][k]), theta));
        ...
        */
        ...
        /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
        {
          double uph = HPL * nu * ph->w / (ggeom[i][j][CENT].g * d3x * pow(L_unit, 3));
          SCATTLOOP {
            dtau_scatt[iscatt] = ((HPL * L_unit / (ME * CL * CL)) * dlam *
                                 alpha_inv_scatt(nu, ph->type, iscatt, &(m_grd[i][j][k])));
            bias_scatt[iscatt] = get_scatt_bias(nu, ph->type, iscatt, &(m_grd[i][j][k]), uph);
          }
          #if REBALANCE
          /*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
          {
            ...
            */
            ...
            bound_all_biases( dtau_scatt, bias_scatt, nu, ph->type, &(m_grd[i][j][k]));

            #if CONSERVATIVE_BOUND
            /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
            SCATTLOOP {
              bias_scatt[iscatt] = bound_bias(bias_scatt[iscatt], nu, ph->type, iscatt, &(m_grd[i][j][k]), uph);
            }
            /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
            #endif
        ...
        xabs = -log(get_rand());
        SCATTLOOP { 
          xscatt[iscatt] = -log(get_rand()) / bias_scatt[iscatt]; 
        }
        ...
        if (scatt_possible) {
          SCATTLOOP {
            scatt_possible = (scatt_possible && (xscatt[iscatt] <= dtau_scatt[iscatt]));
          }
        }
...
#pragma omp atomic
            radG[i][j][k][mu] += 1. / (dt * d3x) * ph->w * kphys_to_num * Kcov[mu];
          }
          #if RADIATION == RADTYPE_NEUTRINOS
          /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
          {
#pragma omp atomic
            radG[i][j][k][RADG_YE] += ((1. / (dt * d3x)) * Ucon_grd[i][j][k][0] * ph->w *
...
#pragma omp atomic
          ...
          // number of superphoton absorption //
#pragma omp atomic
          Nabs[i][j][k]++;

          // number of photon absorption //
#pragma omp atomic
          ...
          if (dtau_abs < 100) {
#pragma omp atomic
            dtau_avg[0][i][j][k] += dtau_abs * (-ph->Kcov[2][0] * ME * CL * CL) * ph->w;

#pragma omp atomic
            en_int_avg[0][i][j][k] += (-ph->Kcov[2][0] * ME * CL * CL) * ph->w;
          }

          // reset weight after absorption //
          ph->w = 0.;
        ...
        // scattering //
          ...
          */
          ...
          // record scattered photon //
          int success = scatter_superphoton(P, extra, phscatt, X, Kcov, Kcon, scatt_to_do);
