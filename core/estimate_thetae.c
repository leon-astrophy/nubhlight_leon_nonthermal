...
double dEdt(double Thetae, void *params) {
  ...
  for (int n = 0; n < nthreads; n++) {
    for (int m = 0; m < Nsph_zone[i][j][k][n]; m++) {
      ...
      // Absorption // 
      double alpha_inv_a = alpha_inv_abs(nu[i][j][k][n][m], type, &micro, theta[i][j][k][n][m]);
      double dtau_a = alpha_inv_a * L_unit * HPL / (ME * CL * CL) * dlam[i][j][k][n][m];
      ...
      double alpha_inv_s = alpha_inv_scatt(nu[i][j][k][n][m], type, interaction, &micro);
      double dtau_s = alpha_inv_s * L_unit * HPL / (ME * CL * CL) * dlam[i][j][k][n][m];
...
#pragma omp parallel for collapse(3)
  ZLOOP {
    double Bcon[NDIM];
    get_fluid_zone(i, j, k, P, extra, &micro, Ucon[i][j][k], Ucov[i][j][k], Bcon, Bcov[i][j][k]);
    Ne[i][j][k]         = micro.Ne;
...
#pragma omp parallel
  {
    ...
    while (ph != NULL) {
      ...
      // out-of-box photon //
      if (i < NG || i > NG + N1 - 1 || j < NG || j > NG + N2 - 1 || k < NG || k > NG + N3 - 1) {
        printf("BAD PH????\n");
        printf("[%i] %i %i %i X[] = %e %e %e %e\n", n, i, j, k, X[0], X[1],  X[2], X[3]);
        for (int mu = 0; mu < 3; mu++) {
          printf("X[%i][] = %e %e %e %e\n", mu, ph->X[mu][0], ph->X[mu][1], ph->X[mu][2], ph->X[mu][3]);
          printf("Kcov[%i][] = %e %e %e %e\n", mu, ph->Kcov[mu][0], ph->Kcov[mu][1], ph->Kcov[mu][2], ph->Kcov[mu][3]);
          printf("Kcon[%i][] = %e %e %e %e\n", mu, ph->Kcon[mu][0], ph->Kcon[mu][1], ph->Kcon[mu][2], ph->Kcon[mu][3]);
        }
      ...
      theta[i][j][k][n][Nsph_cntd[i][j][k][n]] = get_bk_angle(X, Kcon, Ucov[i][j][k], Bcov[i][j][k], Bmag[i][j][k]);
...
#pragma omp for collapse(3)
    ZLOOP {
      ...
      // assign according to root finding results //
    ...
    // finalize root finding //
    gsl_root_fsolver_free(s);

  } // omp parallel

  // Clean up mallocs and reset counters //
  // Leon: better not to play with this OMP ... //
#pragma omp parallel for collapse(3)
  ZLOOP {
    for (int n = 0; n < nthreads; n++) {
      free(w[i][j][k][n]);
