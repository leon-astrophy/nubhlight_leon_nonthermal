...
void diag(int call_code) {
  ...
  if ((call_code == DIAG_INIT || call_code == DIAG_LOG || call_code == DIAG_FINAL) && !failed) {
    ...
    // loop over //
    // Leon: added OMP here //
#pragma omp parallel for collapse(3) reduction(+:rmed) reduction(+:pp) \
                                     reduction(+:e) reduction(max:divbmax) 
...
#if RADIATION
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
#pragma omp parallel for collapse(3) reduction(+:mass_proc) reduction(+:egas_proc) reduction(+:Phi_proc) \
                                     reduction(+:jet_EM_flux_proc) reduction(+:lum_eht_proc) reduction(+:erad_proc)
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
#else
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
#pragma omp parallel for collapse(3) reduction(+:mass_proc) reduction(+:egas_proc) reduction(+:Phi_proc) \
...
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/                                     
  ...
  // Get last shell entirely enclosed by stopx_rad[1] (assume r(X^1) independent of X^2, X^3) //
  int stopi_rad = -1;
  ...
  if (stopi_rad >= 0) {
    // Leon: added OMP here //
#pragma omp parallel for collapse(2) reduction(-:lum_proc)
    JSLOOP(0, N2 - 1) {
      KSLOOP(0, N3 - 1) {
  ...
  double lum_super = 0.;
  // Leon: added OMP here //
#pragma omp parallel for collapse(3) reduction(+:num_super) reduction(+:lum_super)
  ZLOOP {
  ...
  if (call_code == DIAG_INIT || call_code == DIAG_LOG || call_code == DIAG_FINAL) {
    ...
    if (mpi_io_proc()) {
      ...
      /* these are standard */
      fprintf(ener_file, "%10.5g %10.5g %10.5g %10.5g %15.8g %15.8g ", t, rmed, pp, e,
       EOS_adiabatic_constant(P[N1 / 2][N2 / 2][N3 / 2][RHO], P[N1 / 2][N2 / 2][N3 / 2][UU], extra[N1 / 2][N2 / 2][N3 / 2]),
       P[N1 / 2][N2 / 2][N3 / 2][UU]);
      ...
      #if RADIATION == RADTYPE_NEUTRINOS
      /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
      fprintf(ener_file, "%15.8g %15.8g %15.8g ", lepton_tot, dlepton_tot, dlepton_perc);
      /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
      #endif // RADIATION == RADTYPE_NEUTRINOS
      #if TRACERS
      /*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
      ...
      // these are standard timers statistic //
      ...
      /*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
      #endif // NEUTRINO_OSCILLATIONS || LOCAL_ANGULAR_DISTRIBUTIONS
...
/***********************************************************************************/
// Diagnostic routines for failure mode //
...
/***********************************************************************************/
/* diverence of magneic field in the flux_ct scheme */
/* Leon: need to exclude boundary points! */
...
/***********************************************************************************/
/* report any imbalance in loading */
...
#pragma omp atomic
  ...
  /*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
  #endif // COMPLAIN_ON_LEPTON_NONCON
