...
// total number of cells on a cpu //
#define N123G ((N1 + 2 * NG) * (N2 + 2 * NG) * (N3 + 2 * NG)) 

// Fixup parameters //
// Leon: change this to match iharm3d, or EHT MAD? //
// rho_flor = RHOMIN / pow(r, -FLOOR_POWER)        //
#define FLR_POWER1  (2.)
#define FLR_POWER2  (2.)
// TODO change this (FLR_R0)? //
#define FLR_R0      (1.) 
#define RHOMINLIMIT (1.e-20)
#define UUMINLIMIT  (1.e-20) // * RHOMINLIMIT)
#define RHOMIN      (1.e-5)
#define UUMIN       (1.e-7) // * RHOMIN)
...
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
#ifndef NTGAMMAMIN
#define NTGAMMAMIN (1.e2)
#endif
#ifndef NTGAMMAMAX
#define NTGAMMAMAX (1.e12)
#endif
#ifndef PLAW
#define PLAW       (3.5)
#endif
...
// nonthermal electrons as extra passive variables //
#if NONTHERMAL
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
#define NTESTART (NVAR_BASE + NVAR_PASSIVE + NVAR_EL)
#ifndef NTEBINS
#define NTEBINS  (128) // Default bin number
#endif // NTEBINS
...
/* number of variables */
#define NVAR (NVAR_BASE + NVAR_PASSIVE + NVAR_EL + NVAR_NT)
...
#else
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
// TODO: RAD_SCATT_TYPES should be 5, with electrons //
#define RAD_SCATT_TYPES   (4) 
...
// Units //
#define NEED_UNITS (COULOMB || NONTHERMAL || RADIATION || EOS == EOS_TYPE_TABLE)
...
typedef double grid_radtype_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG][RAD_NUM_TYPES];
typedef double grid_fourvector_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG][NDIM];
typedef double grid_radg_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG][NDIM + NRADCOMP];
typedef double grid_tensor_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG][NDIM][NDIM];
typedef int    grid_int_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG];
typedef double grid_eosvar_type[N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG][EOS_NUM_EXTRA];
#if RZ_HISTOGRAMS
/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
extern grid_fourvector_type jcon;  // 4-current 
...
extern double dtau_avg[RAD_SCATT_TYPES + 1][N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG];
extern double en_int_avg[RAD_SCATT_TYPES + 1][N1 + 2 * NG][N2 + 2 * NG][N3 + 2 * NG];
...
typedef double grid_local_angles_type[LOCAL_NUM_BASES][LOCAL_ANGLES_NX1][LOCAL_ANGLES_NX2][RAD_NUM_TYPES][LOCAL_ANGLES_NMU];
...
#if RAD_NUM_TYPES >= 4
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
typedef double grid_Gnu_type[LOCAL_NUM_BASES][LOCAL_ANGLES_NX1][LOCAL_ANGLES_NX2][LOCAL_ANGLES_NMU];
typedef double grid_local_moment_type[LOCAL_NUM_BASES][LOCAL_NUM_MOMENTS][LOCAL_ANGLES_NX1][LOCAL_ANGLES_NX2];
...
// nonthermal variables //
#if NONTHERMAL
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
extern double nteGammas[NTEBINS];      // Lorentz factor
extern double log10nteGammas[NTEBINS]; // log of Lorentz
extern double log10BinSpace;           // bin width in log10
extern double logeBinSpace;            // bin width in natural log
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
#endif
...
#if NONTHERMAL
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/* Leon's patch, electron mass in code unit! */
extern double ME_CODE; 

/* Leon's patch, speed of light in code unit! */
/* Generally redudant but is useful for testing */
extern double CL_CODE; 
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
#endif
...
#if NONTHERMAL
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
extern double normterm;     // normalization
extern double powlaw_int;   // power-law integral
extern double gammainjmin;  // lower injection limit
extern double gammainjmax;  // maximum injection limit
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
#endif
...
struct of_photon {
  // NSUP >=3 X^{\mu}, K^{\mu}, K_{\mu} so photon data always available anywhere between n and n+1 //
  // n = 0: initial step, n = 1: half step, n = 2: final step //
  double X[NSUP][NDIM];
...
struct of_microphysics {
  ...
  /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
  #endif // BURROWS_OPACITIES || HDF5_OPACITIES
  /*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
...
typedef double (*dsdom_ftype)(double, double, const struct of_microphysics *, int, int);
...
#define ZLOOP    for (int i = 0 + NG; i < N1 + NG; i++) \
...
// macros for nonthermal electrons //  
#if NONTHERMAL
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
// loop through nonthermal electrons but using primitive index //
#define NTELOOP for (int ip = NTESTART; ip < NVAR; ip++)

// loop through nonthermal electrons but using lorentz factor index //
...
#define DLOOP3 for (int mu = 0; mu < NDIM; mu++) \
               for (int nu = 0; nu < NDIM; nu++) \
...
#define LOCALXLOOP  for (int i = 0; i < LOCAL_ANGLES_NX1; ++i)   \
...
int rad_mpi_transport(struct of_photon **ph, struct of_photon **prev, struct of_photon **head, double X[NDIM], int active);
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#endif
...
void jac_harm_to_bl(const double X[NDIM], double Jcov[NDIM][NDIM], double Jcon[NDIM][NDIM]);
void jac_bl_to_cart(const double X[NDIM], double Jcov[NDIM][NDIM], double Jcon[NDIM][NDIM]);
...
void apply_rad_force_e(grid_prim_type Prh, grid_prim_type Pr, grid_radg_type radG, double Dt);
/*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
#endif // RADIATION
...
double EOS_pressure_rho0_w(double rho, double w, double gamma, const struct of_geom *geom, double *extra);
...
void   EOS_set_floors(double scale, double rho, double u, double bsq, double *rhoflr, double *uflr, const double *extra);
...
void EOS_SC_set_floors(double scale, double rho, double u, double ye, double bsqr, double *rhoflr, double *uflr);
...
void   EOS_SC_get_polytrope(double lrho, double lT, double ye, double *poly_K, double *poly_gamma);
...
void   EOS_SC_isoentropy_hm1(double hm1, const struct of_adiabat *a, double *lrho_guess, double *rho, double *u);
void   do_ye_fixup( int i, int j, int k, double pv[NVAR], double pv_prefloor[NVAR]);
void do_ye_em_fixup( int i, int j, int k, double pv[NVAR], double pv_prefloor[NVAR]);
void do_atm_fixup(int i, int j, int k, double pv[NVAR], double pv_prefloor[NVAR]);
...
void   estimate_Thetae(grid_prim_type P, grid_eosvar_type extra, double t, double dt);
/*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
#endif
...
void sync_mpi_photons(struct of_photon **ph_mpi, grid_prim_type P, double t, double dt);
...
void get_local_angle_bins(struct of_photon *ph, int *pi, int *pj, int *pmu1, int *pmu2);
void accumulate_local_angles();
#if RAD_NUM_TYPES >= 4
/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
void compute_local_gnu(grid_local_angles_type local_angles, grid_Gnu_type local_Ns, grid_Gnu_type local_wsqr, grid_Gnu_type gnu);
...
// phys.c //
void   primtoflux(double *pr, struct of_state *q, int dir, int magnetic, struct of_geom *geom, double *flux);
void   bcon_calc(double *pr, double *ucon, double *ucov, double *bcon);
void   mhd_calc(double *pr, int dir, int magnetic, struct of_state *q, double *extra, double *mhd);
void   source(double *ph, struct of_geom *geom, int ii, int jj, double *dU, double Dt, double *extra);
...
int push_X_K(double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], 
             grid_prim_type P, grid_prim_type Prad, double KdotKprev, int type, double dtpush);
int push_superphoton(struct of_photon *ph, grid_prim_type P, grid_prim_type Prad, double dtpush);
...
void   list_remove(struct of_photon **ph, struct of_photon **ph_head, struct of_photon **ph_prev);
...
void get_fluid_zone(int i, int j, int k, grid_prim_type Prad, grid_eosvar_type extra, struct of_microphysics *m, double Ucon[NDIM],
...
int    get_X_K_interp(struct of_photon *ph, double t_interp, grid_prim_type P, double X[NDIM], double Kcov[NDIM], double Kcon[NDIM]);
...
// reconstruction.c //
void reconstruct(double ptmp[NMAX + 2 * NG][NVAR], int N, double p_l[NMAX + 2 * NG][NVAR], double p_r[NMAX + 2 * NG][NVAR]);
...
double total_cross_lkup(double w, int type, int interaction, const struct of_microphysics *m);
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#endif
...
#if RADIATION
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
void coord_to_tetrad(double Ecov[NDIM][NDIM], double Kcoord[NDIM], double Ktetrad[NDIM]);
void tetrad_to_coord(double Econ[NDIM][NDIM], double Ktetrad[NDIM], double Kcoord[NDIM]);
...
void   lagrange_interp_grid_3d( double v_interp[], double X[NDIM], double *v, int size);
void lagrange_interp_prim_3d(double P_interp[NVAR], double X[NDIM], grid_prim_type P);
void   push_tracers(double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], grid_prim_type P, grid_prim_type Ph, double dt);
int    tracer_get_id(struct of_photon *ph);
double tracer_get_mass(struct of_photon *ph);
void   tracer_get_X_u(struct of_photon *ph, double t_interp, double X[NDIM], double ucov[NDIM], double ucon[NDIM]);
void   set_tracer(struct of_photon *ph, int id, int nstep, double t, double X[NDIM], double mass, grid_prim_type P);
void   make_tracer(struct of_photon **head, int id, int nstep, double t, double X[NDIM], double mass, grid_prim_type P);
void   sample_tracers_in_cell(struct of_photon **head, int nstep, int i, int j, int k, int *last_id, int ntracers_per_cell, double t, grid_prim_type P);
void   sample_all_tracers(long unsigned int ntracers_tot, int nstep, double t, grid_int_type tcrs_in_cell, grid_prim_type P);
void   prune_tracers();
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
#endif
