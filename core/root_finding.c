...
int find_root(double (*f)(const double, const void *), const void *params,
  ...
  // Next try Secant //
  status = root_secant(f, params, ytarget, xguess, xmin, xmax, xtol, ytol, xroot);
  ...
  #endif
  status = root_bisect(f, params, ytarget, xguess, xmin, xmax, xtol, ytol, xroot);
...
static int root_secant(double (*f)(const double, const void *),
  ...
  *xroot = x;
  ...
  // check root finding results //
  const int secant_failed = ((fabs(x - x_last) > xtol && fabs(frac_error) > ytol) || isnan(x) || isinf(x));
...
static int root_bisect(double (*f)(const double, const void *),
  ...
  // Try to find reasonable region for bisection // 
  do { 
    dx = fabs(grow * x);
    xl = x - dx;
    xr = x + dx;
