...
/*************************************************************************/
/* linear reconstruction with MC limiter */
...
void para_cs(double x1, double x2, double x3, double x4, double x5, double *lout, double *rout) {}
//////////////////////////////////////////////////////////////////////////
/*
#define PARA2LIM 0
...
void para(double x1, double x2, double x3, double x4, double x5, double *lout, double *rout) {
  ...
  double C = 1.25;
  int    i = 2;
  double a[5], amin, amax, D2a, D2aL, D2aR, D2alim, s; 
  ///////////////////////////////////
  //, D2ma, D2ca, D2pa;//alphamax;
  ///////////////////////////////////
  ...
  */
  ...
  if ((a[i] - aimh) * (aiph - a[i]) <= 0. || (a[i + 1] - a[i]) * (a[i] - a[i - 1]) <= 0.) {
    ...
    /////////////////////////////////////////////////////////////////
    double D2alim = 0.;
    if (MY_SIGN(D2a) == MY_SIGN(D2aL) && MY_SIGN(D2a) == MY_SIGN(D2aC) && MY_SIGN(D2a) == MY_SIGN(D2aR)) {
      s      = MY_SIGN(D2a);
...
void reconstruct_lr_lin(double Ptmp[NMAX + 2 * NG][NVAR], int N, double P_l[NMAX + 2 * NG][NVAR], double P_r[NMAX + 2 * NG][NVAR]) {
...
/*****************************************************************************/
/* reconstruct by PPM */
void reconstruct_lr_par(double Ptmp[NMAX + 2 * NG][NVAR], int N, double P_l[NMAX + 2 * NG][NVAR], double P_r[NMAX + 2 * NG][NVAR]) {
  ISLOOP(-1, N) {
    PLOOP {
      para(Ptmp[i - 2][ip], Ptmp[i - 1][ip], Ptmp[i][ip], Ptmp[i + 1][ip], Ptmp[i + 2][ip], &P_l[i][ip], &P_r[i][ip]);
      //////////////////////////
      /*para2(Ptmp[i-2][ip],
           Ptmp[i-1][ip],
           Ptmp[i][ip],
...
// main reconstruction subroutine 
void  reconstruct_lr_ppm(double q[NMAX + 2 * NG][NVAR], int N, double P_l[NMAX + 2 * NG][NVAR], double P_r[NMAX + 2 * NG][NVAR]) {
  ...
  PLOOP {
    ...
    ISLOOP(-2, N + 1) {
      dph[i] = (7. * (q[i - 1][ip] + q[i][ip]) - (q[i + 1][ip] + q[i - 2][ip])) / 12.;
    ...
    ISLOOP(-1, N) {
      ...
      if (qa <= 0. || qb <= 0.) {
        ...
        qc = d2qc[i + 1];
        qd = d2qf[i];
        if (MY_SIGN(qa) == MY_SIGN(qb) && MY_SIGN(qa) == MY_SIGN(qc) && MY_SIGN(qa) == MY_SIGN(qd)) {
          qe = MY_SIGN(qd) * MY_MIN(MY_MIN(C * fabs(qa), C * fabs(qb)), MY_MIN(C * fabs(qc), fabs(qd)));
        } else {
          qe = 0.;
        }
        ...
        qb = MY_MAX(fabs(q[i][ip]), MY_MAX(fabs(q[i + 1][ip]), fabs(q[i + 2][ip])));
...
/*****************************************************************************/
/* reconstruct by WENO method */
void reconstruct_lr_weno(double Ptmp[NMAX + 2 * NG][NVAR], int N, double P_l[NMAX + 2 * NG][NVAR], double P_r[NMAX + 2 * NG][NVAR]) {
  ISLOOP(-1, N) {
    PLOOP {
      weno(Ptmp[i - 2][ip], Ptmp[i - 1][ip], Ptmp[i][ip], Ptmp[i + 1][ip], Ptmp[i + 2][ip], &P_l[i][ip], &P_r[i][ip]);
    }
  }
}

/*****************************************************************************/
/* reconstruct by MP5 method */
void reconstruct_lr_mp5(double Ptmp[NMAX + 2 * NG][NVAR], int N, double P_l[NMAX + 2 * NG][NVAR], double P_r[NMAX + 2 * NG][NVAR]) {
  ISLOOP(-1, N) {
    PLOOP {
      mp5(Ptmp[i - 2][ip], Ptmp[i - 1][ip], Ptmp[i][ip], Ptmp[i + 1][ip], Ptmp[i + 2][ip], &P_l[i][ip], &P_r[i][ip]);
    }
  }
}
