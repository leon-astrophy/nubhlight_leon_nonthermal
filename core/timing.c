...
/*********************************************************************/
/* get physical time spent per time step */
...
void report_performance() {
  if (mpi_myrank() == 0) {
    ...
    fprintf(stdout, "   ELECTRON: %8.4g s (%.4g %%)\n",
        times[TIMER_ELECTRON] / dnstep,       100. * times[TIMER_ELECTRON] / times[TIMER_ALL]);
    #endif
    #if RADIATION
    /*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
    fprintf(stdout, "   MAKE:     %8.4g s (%.4g %%)\n",
        times[TIMER_MAKE] / dnstep,           100. * times[TIMER_MAKE] / times[TIMER_ALL]);
    fprintf(stdout, "   PUSH:     %8.4g s (%.4g %%)\n",
        times[TIMER_PUSH] / dnstep,           100. * times[TIMER_PUSH] / times[TIMER_ALL]);
    fprintf(stdout, "   INTERACT: %8.4g s (%.4g %%)\n",
        times[TIMER_INTERACT] / dnstep,       100. * times[TIMER_INTERACT] / times[TIMER_ALL]);
    #if NEUTRINO_OSCILLATIONS || LOCAL_ANGULAR_DISTRIBUTIONS
    /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
    fprintf(stdout, "   OSCILL:   %8.4g s (%.4g %%)\n",
        times[TIMER_OSCILLATIONS] / dnstep,   100. * times[TIMER_OSCILLATIONS] / times[TIMER_ALL]);
    /*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&*/
    #endif // NEUTRINO_OSCILLATIONS || LOCAL_ANGULAR_DISTRIBUTIONS
    fprintf(stdout, "   MICRPHYS: %8.4g s (%.4g %%)\n",
        times[TIMER_MICRO] / dnstep,          100. * times[TIMER_MICRO] / times[TIMER_ALL]);
    /*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
    #endif // RADIATION
