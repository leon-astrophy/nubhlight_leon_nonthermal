...
void get_K_source(double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], double src[NDIM]);
...
/******************************************************************************/
/* push superphoton */
int push_X_K(double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], grid_prim_type P, 
             grid_prim_type Prad, double KdotKprev, int type, double dtpush) {
...
int push_superphoton(struct of_photon *ph, grid_prim_type P, grid_prim_type Prad, double dtpush) {
  ...
  // perform push //
  int status    = push_X_K(ph->X[2], ph->Kcov[2], ph->Kcon[2], P, Prad, ph->KdotKprev, ph->type, dtpush);
...
#pragma omp parallel reduction(+:step_lost_local) reduction(+:step_tot_local) reduction(+:tracer_tot_local)
  {
  ...
  // record total pushed superphoton //
...
void push(double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], double dt) {
  ...
  // quick way of initializing an array //
  ...
  // get source term in geodesic equation //
...
/******************************************************************************/
/* source term in the geodesic equation */
void get_X_source(double Kcon[NDIM], double src[NDIM]) {
  for (int mu = 0; mu < NDIM; mu++) {
