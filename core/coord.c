...
// given r, get the index in terms of the native coordinate // 
...
/*****************************************************************************/
/* given native coordinate, get theta */
...
#if DEREFINE_POLES
/*****************************************************************************/
/* given native coordinate, given theta */
...
/*****************************************************************************/
/* given native coordinate, give radius */
...
/*****************************************************************************/
/* given native coordinate, get r and theta */
...
void jac_harm_to_bl(const double X[NDIM], double Jcov[NDIM][NDIM], double Jcon[NDIM][NDIM]) {
  ...
  // choose if we define theta resolution at pole //
  #if DEREFINE_POLES
  {
    double thG = thG_of_X(X);
    double y, thJ;
    ...
    dthdX2 = dthGdX2 + exp(mks_smooth * (startx[1] - X[1])) * (dthJdX2 - dthGdX2);
  }
  #else
  {
    dthdX1 = 0.0;
...
/****************************************************************************/
/* jacobian to map from native to cartesian */
...
void set_grid() {
  ...
  // loop //
  // Leon: set_grid() is called only once, so neglect parallization //
  ISLOOP(-NG, N1 - 1 + NG) {
    JSLOOP(-NG, N2 - 1 + NG) {

      // loop variable is loc // 
      LOCLOOP { 
        coord(i, j, 0, loc, X);
      ...
      // loop over to find the light phase speed //
...
/****************************************************************************/
/* given cartesian coordinate (not native), get spherical */
...
/* zero-out all arrays */
void zero_arrays() {
  // Leon: zero_arrays() is called only once, so neglect parallization //
