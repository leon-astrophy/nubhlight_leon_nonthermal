...
void   init_hc_table(hc_ftype f, double table[NW + 1][NT + 1], const char *hc_name);
...
double hc_quad_max();
double nu_cross_factor(double sigma, int type, int interaction, const struct of_microphysics *m);
double total_cross_ions(double sigma_hc, double A, double Z);
double nu_cross_delta(int type, int interaction);
double nu_cross(double w, double mu, const struct of_microphysics *m, int type,int interaction);
double nu_cross_max(const struct of_microphysics *m, int type, int interaction);

// multiscatt test //
#if MULTISCATT_TEST
double hc_flat(double we, double mue);
double ms_flat(double w, double mu, const struct of_microphysics *m, int type, int interaction);
...
/************************************************************************************************/
/* perform scattering of super photon */
int scatter_superphoton(grid_prim_type P, grid_eosvar_type extra, struct of_photon *ph, 
    double X[NDIM], double Kcov[NDIM], double Kcon[NDIM], int interaction) {
  ...
  // return if scattering particle is too cold //
  ...
  // convert from coordinate to tetrad frame //
  ...
  DLOOP1 {
    if (is_practically_nan(Pelectron[mu])) {
      ...
      printf("k.k = %e\n", -Ktetrad[0] * Ktetrad[0] + Ktetrad[1] * Ktetrad[1] +
                            Ktetrad[2] * Ktetrad[2] + Ktetrad[3] * Ktetrad[3]);
      return fail;
    }
  }

  // perform sampling //
  sample_scattered_rad(Ktetrad, Pelectron, Ktetrad_scatt, &m, ph->type, interaction);
...
/************************************************************************************************/
/* sample scattering cross section */
void sample_cross_section(double k, double *k0p, double *cth,
     const struct of_microphysics *m, int type, int interaction) {
  #if RADIATION == RADTYPE_LIGHT
  /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
...
/************************************************************************************************/
/* initialize hor cross section table */
void init_hc_table(hc_ftype f, double table[NW + 1][NT + 1], const char *hc_name) {
...
/************************************************************************************************/
/* interpolate hot cross section table */
...
double total_cross_lkup(double w, int type, int interaction, const struct of_microphysics *m) {
  ...
  /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
  {
    ...
    double sigma = interpolate_hc_table(w, thetae, hc_klein_nishina, table[0], HOTCROSS[0]);
  ...
  /*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
  {
    ...
    /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
    {
      double sigma = interpolate_hc_table(w, thetae, hc_flat, table[0], HOTCROSS[0]);
      return 4 * M_PI * sigma * ms_flat(w, 0, m, type, interaction) / hc_flat(w, 0);
    }
    /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
    #else  // Normal neutrino scattering
    /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
    {
      ...
      double sigma = interpolate_hc_table(w, thetae, hc_quad, table[0], HOTCROSS[0]);
      return nu_cross_factor(sigma, type, interaction, m);
    }
    /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
    #endif // neutrino scattering type
...
double total_cross_num(hc_ftype f, double w, double thetae) {
  ...
  for (mue = -1. + 0.5 * dmue; mue < 1.; mue += dmue)
    for (gammae = 1. + 0.5 * dgammae; gammae < 1. + MAXGAMMA * thetae; gammae += dgammae) {
...
/************************************************************************************************/
/* hot cross section klein nishina */
double hc_klein_nishina(double we, double mue) {
  double sigma;
  if (we < 1.e-3) {
