...
/**********************************************************************/
/* find the array index of a given value */
...
double interp_1d(double x, const double xmin, const double xmax, const int imin, const int imax, 
                 const double *restrict tab_x, const double *restrict tab_y) {
...
void set_units() {
  ...
  // Leon: I moved T_unit back to the block of METRIC == MKS //
  // Leon: seem to be a bad idea //
  #if METRIC == MKS
  ...
  // Nonthermal electron module needs ME and CL //
  #if NONTHERMAL
  /*!!!!!!!!!!!!!!!!!!!!!!!!!!*/
  if(T_unit == 0.0) {T_unit = 1.0;}
  ME_CODE = ME/M_unit;
  CL_CODE = CL/(L_unit/T_unit);
  /*!!!!!!!!!!!!!!!!!!!!!!!!!!*/
  #endif
